...
/* GPS Waypoints */
int waypoint_num = 0;
const int waypoint_total = 10;	// <- should always be the same number of GPS waypoints
double gps_array[10][2] = {{40.06519806873442,-105.20985681744139},
...
void set_speed(void) {	// test this and delete the delays after it works
	if(waypoint_num >= 2 && waypoint_num < 3) speed.write(110);
	else {
...
double servo_angle;
	if(waypoint_num >= 2 && waypoint_num < 3 && waypoint_distance > 20) waypoint_heading = 290.0; 
