...
- debug the shutoff function in the "waypoint" function
...
double lat_array[7] = {39.53872, 39.53864, 39.53872, 39.53844, 39.53818, 39.53844, 39.53871};
double lon_array[7] = {-105.01675, -105.01662, -105.01675, -105.01642, -105.0166, -105.01642, -105.01674};
//double waypoint_angle[ ] = {0, 90.0, 
int waypoint_num = 0;
const int waypoint_total = 7;
...
/*	if(waypoint_distance > 10) speed.write(100);	delay(3000);
	if(waypoint_distance > 25) speed.write(110);	delay(3000);
	if(waypoint_distance > 50) speed.write(120);	delay(3000);
	if(waypoint_distance > 75) speed.write(150);	delay(3000);
	if(waypoint_distance > 100) speed.write(180);	delay(3000);
*/
	speed.write(110);
...
double servo_angle;
	...
	steering.write(180.0-servo_angle);
	return;
}
...
void waypoint(void) {				// Distance and angle to next waypoint
	...
	if(waypoint_heading < 0)
		waypoint_heading += 360.0;

	if (waypoint_distance < 10)
		waypoint_num++;

	while (waypoint_num >= waypoint_total) {		// should shut off the vehicle by disabling autonomous mode & MUX'r
		autonomous = false;
		digitalWrite(STROBE, LOW);
...
void compass(void) {				// Determine compass heading with 0-deg matching standard graph
	...
	y = (yParam - COMP_Y);
	northFi = atan2(y,x);
	compass_heading = (northFi * 180.0) / M_PI + 90.0 - 9.0; // 9 is to correct for magnetic north
