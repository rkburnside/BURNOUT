...
/* GPS Waypoints */
int waypoint_num = 0;
const int waypoint_total = 16;	// <- should always be the same number of GPS waypoints
double gps_array[16][2] = {{39.538298770540344,-105.01172583175334},
...
void set_speed(void) {	// test this and delete the delays after it works
	if(waypoint_num >= 2 && waypoint_num < 7) speed.write(110);
	else {
		if(waypoint_distance <= 15)		speed.write(105);
		if(waypoint_distance > 15)		speed.write(110);
	}
	return;
}
...
void u_sonic(void) {				// Ultrasonic Distance Sensor
	...
	if((u_distance < 6) || (u_distance2 < 15)) {object_detected = true;}
	else {object_detected = false;}
//	if(object_detected) Serial.println("true");
//	else Serial.println("false");
	return;
}

/* GPS Routes
SPARKFUN ACTUAL ROUTE
...
Stake Center Route - Building
{{39.53869644189598,-105.011342275864},
{39.538756428534754,-105.0113261826099},
{39.53879159309199,-105.01129667831074},
{39.538909497653925,-105.01116793227803},
{39.53901189173939,-105.01106634361126},
{39.5390749808472,-105.0110019705949},
